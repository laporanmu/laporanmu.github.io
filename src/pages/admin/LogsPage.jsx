...
import {
    faShieldHalved, faRotateRight, faDownload, faFilter, faSearch,
    ...
    faPlus, faPen, faTrash, faSync, faTools
...
import { ActionBadge, DiffViewer, JsonVisualizer } from '../../components/ui/AuditDetails'
...
// Shared via components/ui/AuditDetails

const ActivityTrends = ({ data, loading }) => {
    // Process last 24h activity
...
// Replaced by components/ui/AuditDetails
...
const LogRow = ({ entry, isExpanded, onToggle, onRestore }) => {
    ...
    return (
        <div className={`transition-all ${isExpanded ? 'bg-[var(--color-surface-alt)]/50' : 'hover:bg-[var(--color-surface-alt)]/30'}`}>
            <div className="flex flex-col md:flex-row md:items-center gap-3 px-5 py-4 cursor-pointer border-b border-[var(--color-border)]" onClick={onToggle}>
                {/* Time & Action */}
                <div className="w-40 shrink-0">
                    <p className="text-[11px] font-black text-[var(--color-text)] tabular-nums">{fmtRelative(entry.created_at)}</p>
                    <p className="text-[9px] text-[var(--color-text-muted)] mt-0.5 tabular-nums">{fmtDate(entry.created_at)} • {fmtTime(entry.created_at)}</p>
                </div>
                        ...
                        <div className="min-w-0">
                            ...
                            >
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <FontAwesomeIcon
                                        icon={faInfoCircle}
                                        className="text-[10px] text-[var(--color-text-muted)] opacity-30 group-hover:opacity-100 transition-opacity"
                                    />
                                </div>
                                <span className="text-[10px] text-[var(--color-text-muted)] font-mono opacity-60">
                ...
                <div className="hidden md:flex flex-col items-end gap-1.5 w-24">
                    ...
                    <FontAwesomeIcon icon={isExpanded ? faXmark : faAngleDown} className="text-[10px] text-[var(--color-text-muted)] opacity-30" />
                </div>
            </div>
                ...
                <div className="px-5 py-4 bg-[var(--color-surface)] border-b border-[var(--color-border)] space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {entry.action === 'DELETE' && onRestore && (
                        <div className="pt-2 border-t border-[var(--color-border)] flex justify-end">
                            <button onClick={() => onRestore(entry)} className="h-8 px-4 rounded-xl bg-amber-500 text-white text-[10px] font-black hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 flex items-center gap-2">
...
export default function LogsPage() {
    ...
    const [stats, setStats] = useState({ total: 0, security: 0, master: 0, operational: 0 })
    ...
    const [isLive, setIsLive] = useState(() => {
        try {
            return localStorage.getItem('audit_live_mode') === 'true'
        } catch { return false }
    })
    ...
    useEffect(() => {
        localStorage.setItem('audit_live_mode', isLive)
    }, [isLive])

    const fetchLogs = useCallback(async (quiet = false, isInitial = false) => {
        if (!isAllowed) return
        ...
        try {
            ...
            // UUID Regex for validation to avoid 400 errors
    ...
    const handleRestore = async () => {
        ...
        try {
            ...
            setRestoring(false); setRestoreEntry(null)
        }
    }
    ...
    const handleCleanup = async () => {
        ...
        try {
            ...
            const { error, count } = await supabase.from('audit_logs').delete().lt('created_at', cutoff.toISOString())
            if (error) throw error
            addToast(`Pembersihan berhasil. Data lama dihapus ✓`, 'success')
            await logAudit({ 
                action: 'DELETE', 
        ...
        <DashboardLayout title="Audit Logs">
            <div className="p-4 md:p-6 space-y-4 max-w-7xl mx-auto">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-[11px] font-medium text-[var(--color-text-muted)] opacity-60">Jejak digital lengkap aktivitas sistem dan akses data.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 glass rounded-2xl p-5 border border-[var(--color-border)] relative overflow-hidden">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p className="text-[9px] text-[var(--color-text-muted)] opacity-60 mt-0.5">Frekuensi perubahan data per jam.</p>
                            </div>
                            {autoRefresh && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            ...
                            {
                                label: 'Security',
                                value: logs.filter(l => l.source === 'SYSTEM').length,
                                icon: faShieldHalved,
                                color: 'text-rose-500',
                                bg: 'bg-rose-500/10',
                                sub: 'Akses & Auth'
                            ...
                            {
                                label: 'Master Data',
                                value: logs.filter(l => ['students', 'teachers', 'classes'].includes(l.table_name)).length,
                                icon: faUsers,
                                color: 'text-emerald-500',
                                bg: 'bg-emerald-500/10',
                                sub: 'Siswa, Guru, Kelas'
                            },
                            {
                                label: 'Operational',
                                value: logs.filter(l => l.source === 'OPERATIONAL').length,
                                icon: faBolt,
                ...
                <div className="glass rounded-2xl border border-[var(--color-border)] overflow-hidden">
                    <div className="flex flex-wrap items-center gap-2 p-3">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => setAutoRefresh(!autoRefresh)} className={`h-10 px-4 rounded-xl border text-[11px] font-black flex items-center gap-2 transition-all ${autoRefresh ? 'bg-emerald-500 text-white border-transparent' : 'border-[var(--color-border)] bg-[var(--color-surface-alt)]'}`}>
                                <FontAwesomeIcon icon={faCircle} className={`text-[8px] ${autoRefresh ? 'animate-pulse' : 'opacity-20'}`} /> {autoRefresh ? 'Live' : 'Static'}
                            </button>
                            <button onClick={() => fetchLogs()} className="h-10 px-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] hover:bg-[var(--color-surface)] text-[11px] font-black flex items-center gap-2 transition-all">
                                <FontAwesomeIcon icon={faRotateRight} className={loading ? 'animate-spin' : ''} /> <span className="hidden sm:inline">Refresh</span>
                            </button>
                            <button onClick={() => setShowFilters(!showFilters)} className={`h-10 px-4 rounded-xl border font-black text-[11px] flex items-center gap-2 transition-all ${showFilters ? 'bg-[var(--color-primary)] text-white border-transparent' : 'border-[var(--color-border)] bg-[var(--color-surface-alt)]'}`}>
                                <FontAwesomeIcon icon={faFilter} /> Filter
                            </button>
                            <div className="h-8 w-px bg-[var(--color-border)] mx-1" />
                            <div className="flex items-center gap-1.5">
                                <button onClick={() => handleExport('csv')} disabled={isExporting} className="h-10 w-10 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)] hover:text-emerald-500 transition-all flex items-center justify-center" title="Export CSV">
                                    <FontAwesomeIcon icon={faFileExport} />
                                </button>
                                <button onClick={() => setIsCleanupOpen(true)} className="h-10 w-10 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-500 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center" title="Cleanup Logs">
                        ...
                        <div className="px-3 pb-3 border-t border-[var(--color-border)] pt-4 flex flex-wrap gap-4 items-end animate-in fade-in duration-200">
                            <div className="space-y-1.5">
                                ...
                                <div className="flex gap-1 bg-[var(--color-surface-alt)] p-1 rounded-xl border border-[var(--color-border)]">
                                    {[{ v: '', l: 'Audit' }, { v: 'SYSTEM', l: 'Security' }, { v: 'MASTER', l: 'Master' }, { v: 'OPERATIONAL', l: 'Ops' }].map(opt => (
                            ...
                            <button onClick={() => { setFilterSource(''); setFilterTable(''); setFilterRange({ from: '', to: '' }); setFilterAction('') }}
                    ...
                    <div className="w-full max-w-sm bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="p-6">
                            ...
                            <p className="text-[11px] text-[var(--color-text-muted)] mb-4 leading-relaxed">
                                Anda akan mengembalikan data yang dihapus ke tabel <span className="font-bold text-amber-600 underline">{TABLE_LABELS[restoreEntry.table_name] || restoreEntry.table_name}</span>.
                                Tindakan ini sendiri akan dicatat sebagai entri Audit baru.
                            </p>
                            <div className="flex gap-2 pt-2">
                                <button onClick={() => setRestoreEntry(null)} className="flex-1 h-10 rounded-xl border border-[var(--color-border)] text-[11px] font-black hover:bg-[var(--color-surface-alt)] transition-all">Batal</button>
                        ...
                        <div className="p-6">
                            ...
                            <div className="space-y-3 mb-6">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {[30, 90, 180].map(d => (
                                        <button key={d} onClick={() => setCleanupDays(d)} className={`h-9 rounded-xl border text-[10px] font-black transition-all ${cleanupDays === d ? 'bg-rose-500 border-rose-500 text-white shadow-lg shadow-rose-500/20' : 'border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]'}`}>
                                            {d} Hari
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button onClick={() => setIsCleanupOpen(false)} className="flex-1 h-10 rounded-xl border border-[var(--color-border)] text-[11px] font-black hover:bg-[var(--color-surface-alt)] transition-all">Batal</button>
