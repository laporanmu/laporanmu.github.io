                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-[var(--color-surface-alt)] border-b border-[var(--color-border)]">
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)]">Nama Kelas</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)] text-center">Tingkat</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)] text-center">Program</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)] text-center">Gender</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)]">Wali Kelas</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)] text-center">Siswa</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-[var(--color-text-muted)] text-right">Aksi</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[var(--color-border)]">
                                {filteredClasses.map((cls) => (
                                    <tr key={cls.id} className="hover:bg-[var(--color-surface-alt)]/50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <span className="font-bold text-[var(--color-text)]">{cls.name}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="badge badge-primary px-3 py-1 text-xs">Level {cls.grade}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            {cls.major.includes('Boarding') ? (
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400 text-xs font-bold border border-amber-200/50">
                                                    <FontAwesomeIcon icon={faBed} className="text-[10px]" /> Boarding
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 text-xs font-bold border border-slate-200/50">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            {cls.major.includes('Putra') ? (
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400 text-xs font-bold border border-blue-200/50">
                                                    <FontAwesomeIcon icon={faMars} className="text-[10px]" /> Putra
                                                </span>
                                            ) : cls.major.includes('Putri') ? (
                                                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-pink-50 text-pink-600 dark:bg-pink-900/20 dark:text-pink-400 text-xs font-bold border border-pink-200/50">
                                                    <FontAwesomeIcon icon={faVenus} className="text-[10px]" /> Putri
                                                </span>
                                            ) : (
                                                <span className="text-[var(--color-text-muted)] text-xs font-medium italic">— Campur —</span>
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2 text-[var(--color-text)]">
                                                <FontAwesomeIcon icon={faUserTie} className="text-xs text-[var(--color-text-muted)]" />
                                                <span className="font-medium">{cls.teacherName}</span>
                                            </div>
                                            <div className="text-[10px] text-[var(--color-text-muted)] flex items-center gap-1 mt-0.5 ml-5 font-mono">
                                                <FontAwesomeIcon icon={faCalendarAlt} className="text-[8px]" />
                                                {cls.academicYearName}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <div className="inline-flex items-center gap-2 bg-[var(--color-surface-alt)] px-3 py-1 rounded-full text-sm font-bold text-[var(--color-text)] border border-[var(--color-border)]">
            ...
            <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Konfirmasi Hapus">
                <div className="space-y-6">
                    ...
                    <p className="text-[var(--color-text)] leading-relaxed px-2">
                        Apakah Anda yakin ingin menghapus kelas <strong className="text-red-600 underline underline-offset-4">{itemToDelete?.name}</strong>?
                        <br />
