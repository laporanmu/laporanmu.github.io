    ...
    <DashboardLayout>
      <div className="p-4 md:p-6 max-w-[1800px] mx-auto">
        ...
        <StatsCarousel count={4} className="mb-4">
          {[
            { label: tp('totalToday'), value: stats.total, icon: ClipboardList, bg: 'bg-indigo-500/10 text-indigo-500', border: 'bg-indigo-500' },
            { label: tp('siswaGuruKeluar'), value: stats.keluar, icon: LogOut, bg: 'bg-rose-500/10 text-rose-500', border: 'bg-rose-500' },
            { label: tp('tamuDiDalam'), value: stats.dalamTamu, icon: Building2, bg: 'bg-emerald-500/10 text-emerald-500', border: 'bg-emerald-500' },
            { label: tp('kunjunganTamu'), value: stats.tamu, icon: Users, bg: 'bg-amber-500/10 text-amber-500', border: 'bg-amber-500' },
          ].map((s, i) => (
            <StatCard
              key={i}
              icon={s.icon}
